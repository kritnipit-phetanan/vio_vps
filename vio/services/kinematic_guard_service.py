...
class KinematicGuardService:
    ...
    def apply(self, t: float):
        ...
        if mismatch <= warn_th and not self._hard_active:
            return
        ...
        hard_condition = bool(self._hard_active or (hard_hold_sec <= 1e-9 and mismatch >= hard_th))
        if hard_condition:
        ...
        # Clamp only when hard mismatch condition is persistent/active.
        v_now = np.array(kf.x[3:6, 0], dtype=float).reshape(3,)
        v_now_norm = float(np.linalg.norm(v_now))
        if hard_condition and np.isfinite(v_now_norm) and max_state_speed > 1e-3 and v_now_norm > max_state_speed:
        ...
        if hasattr(runner, "output_reporting"):
            try:
                runner.output_reporting.log_convention_check(
                    ...
                    note=f"dt={dt:.3f},inflate={inflate:.3f},hard_active={int(hard_condition)}",
                )
            except Exception:
                pass
