...
class ExtendedKalmanFilter:
    ...
    Error state uses 3D rotation vector instead of 4D quaternion.
    """
    ...
    def update(self, z, HJacobian, Hx, R=None, args=(), hx_args=(),
        ...
        if dx.shape[0] > 8 and abs(dx[8, 0]) > 0.001:
            print(f"[ESKF-DEBUG] dx[8] (yaw)={np.degrees(dx[8,0]):.2f}°, "
                  f"y={np.degrees(self.y[0,0]):.2f}°, K[8]={self.K[8,0]:.4f}")
    ...
    def _apply_error_state_correction(self, dx):
        """Apply error-state correction with proper manifold updates."""
        dp = dx[0:3, 0]
        dv = dx[3:6, 0]
        dtheta = dx[6:9, 0]
        dbg = dx[9:12, 0]
        ...
        # Multiplicative correction for quaternion
        q_old = self.x[6:10, 0]
